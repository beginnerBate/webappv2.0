...
window.onload = function () {
    ...
    Observer.regist('pageLoad',function(e){
			var index = e.args.toEle
						...
						Array.prototype.slice.call(btn)[index].classList.add('active')
    	switch (index) {
    		case 0:
    		temp.init()
    		break;    		
  ...
  function startTimer(){
		...
		navtimer=setInterval(function(){
			...
			page[router.toEle].classList.add('active')
			return 
			}
			if (router.curEle == 1) {
				// 注册事件 页面切换
	...
	document.ontouchmove=document.ontouchend=startTimer
	startTimer()
}
